...
{
  std::string file_name = ros::package::getPath("roborts_vision") + "/detect_factory/armor/HYY/constraint_set.xml";
...
void ArmorDetector::ArmorFinder(cv::Mat &img)
{
    ...
    for (size_t i = 0; i < iEnd; ++i) {
        for (size_t j = i + 1; j < jEnd; ++j) {
            ...
            float score = squareRatio(l1, l2) * 5.0f
                          + yDis(l1, l2) * 8.0f
                          + shapeSimilarity(l1, l2) * 1.2f  //3.0
                          + parallel(l1, l2) * 6.0f         //1.2
    ...
    if(Flights.size() == 2 && winnerScore > 50) return;
    if(Flights.size() == 3 && winnerScore > 22) return;
    if(winnerScore > 80) return;
...
void ArmorDetector::ArmorFliter(std::vector<cv::RotatedRect> &armors )	// SVM to confirm armor and get the armor number 
{
        ...
        // ROS_ERROR("angle: %f", armors[0].angle);
        // ROS_ERROR("ratio: %f", armor_aspect_ratio);
        // ROS_ERROR("size:  %f", armors[0].size.area());
        
        
        if (0.95 >= armor_aspect_ratio || (armor_aspect_ratio > 2.5 && armor_aspect_ratio < 3.0) || armor_aspect_ratio > 5.0 )
        {
            armors.clear();
            return;
        }
        else if (!(std::abs(armors[0].angle) < 18) )    //40
        {
            armors.clear();
            return;
        }
        else if (!(armors[0].size.area() > 160 && armors[0].size.area() < 223000) )
        {
            armors.clear();
            return;
        }
...
float ArmorDetector::parallel(cv::RotatedRect light1, cv::RotatedRect light2) {
    ...
    if (light1.angle < -1.0 && light2.angle > 1.0)
    {
        // ROS_ERROR("light1: %f", light1.angle);
        // ROS_ERROR("light2: %f", light2.angle);
        return 1000;
    }
    else if (light1.angle > 1.0 && light2.angle < -1.0)
    {
        // ROS_ERROR("light1: %f", light1.angle);
        // ROS_ERROR("light2: %f", light2.angle);
        return 1000;
    }
    else if (std::abs(light1.angle - light2.angle) > 15 )
    {
        // ROS_ERROR("light1: %f", light1.angle);
        // ROS_ERROR("light2: %f", light2.angle);
        return 1000;
    }
    else if (std::abs(light1.angle) > 15 || std::abs(light2.angle) > 15)
    {
        return 1000;
    }
    else
...
float ArmorDetector::armor_size(cv::RotatedRect light1, cv::RotatedRect light2){
    ...
    return (50000 / (X * H));  //111500 gongye //3600 realsense     //yap ce shi !!!!
}   
