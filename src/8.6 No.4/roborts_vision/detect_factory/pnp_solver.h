...
public:
    PnpSolver(float width, float height)
    {
        ...
        std::string file_name = ros::package::getPath("roborts_vision") + "/camera/mercure/mercure_pnp.xml";
    ...
    bool GetXYZ_RUNE(cv::RotatedRect & rect, 
                cv::Point3f & target_3d, 
                cv::Point2f offset = cv::Point2f())
    {
        if (rect.size.height < 1) return false;
        ...
        float size_z = 710 / target_3d.z;

        target_3d.x = size_z * target_3d.x;
        target_3d.y = size_z * target_3d.y;
        target_3d.z = 710;
        if (target_3d.z < min_distance || max_distance < target_3d.z) {
            ROS_WARN ("pnpsolver, out of distance range.");
            return false;
